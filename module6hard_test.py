...
assert Figure.sides_count == 6, "Figure: sides_count должен быть равен 6"
try:
    ...
    print("Вы забыли создать конструктор")
...
assert f2.get_color() == [0, 0, 0], "У вас нельзя записать 0 в качестве значения цвета в конструкторе"
assert f3.get_color() == [0, 0, 0], "У вас сейчас можно при создании объекта записать числа, больше 255 в цвете"
assert len(f1._Figure__sides) == 6, "В конструкторе длина списка sides не равна 6"
assert len(f1.get_sides()) == 6, "Геттер get_sides выдает список не из 6 элементов"
assert isinstance(f1.get_sides(), list), "get_sides должен возвращать список сторон"
assert f1.get_sides() == [
    1]*6 or f5.get_sides() == [1, 1, 1, 1, 1, 1], "Если в конструкторе передано сторон меньше, чем sides_count, то sides должен состоять из 6 единиц: [1,1,1,1,1,1] по количеству sides_count (не создавать просто [1,1,1,1,1,1])"
assert f4.get_sides() == [5, 6, 7, 8, 9, 10], "Обратите внимание, что в конструктор передается *sides - неограниченное количество сторон, и если количество переданных сторон совпадает с sides_count, то стороны записываются в список sides. Метод get_sides должен выдавать именно их."
try:
...
assert cube.get_sides() == [1] * 12, "Когда передается несколько сторон в конструктор, список должен состоять из 12-ти единиц"
cube = Cube((2,2,2), 3)
assert cube.get_sides() == [3] * 12, "Нужно переопределить конструктор. После вызова родительского конструктора - self.set_sides(*list(sides)*12)"
